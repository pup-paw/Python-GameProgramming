...
enemy_y_pos = screen_height/2 - enemy_height / \
    2  # 화면 세로 크기 가장 아래에 해당하는 곳에 위치 (세로)

# 이벤트 루프 (이벤트 루프가 있어야 창이 꺼지지 않음)
running = True  # 게임이 진행중인가?
while running:
    dt = clock.tick(60)  # 게임화면의 초당 프레임 수를 설정 (높을수록 부드러움)
    # 캐릭터가 100만큼 이동을 해야함
    # 10fps : 1초 동안에 10번 동작 -> 1번에 10만큼 이동 -> 10 * 10 = 100
    # 20fps : 1초 동안에 20번 동작 -> 1번에 5만큼 이동 -> 20 * 5 = 100
    ...
    for event in pygame.event.get():  # 어떤 이벤트가 발생하였는가?
        ...
        if event.type == pygame.KEYUP:  # 방향킬ㄹ 떼면 멈춤
    ...
    pygame.display.update()  # 게임화면을 다시 그리기 (계속 업데이트 필요)
